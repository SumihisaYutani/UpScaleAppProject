    ...
    def extract_frames_parallel(self, video_path: Path, total_frames: int, duration: float,
        ...
        if self.gpu_extractor and self.gpu_extractor.is_gpu_acceleration_available():
            try:
                ...
                logger.info("Using GPU acceleration for frame extraction")
                ...
                if progress_dialog:
                    progress_dialog.window.after(0, 
                        lambda: progress_dialog.add_log_message(f"SUCCESS: GPU extraction completed! Estimated speedup: 3-5x"))
                ...
                if progress_dialog:
                    progress_dialog.window.after(0, 
                        lambda: progress_dialog.add_log_message(f"WARNING: GPU extraction failed: {str(e)}"))
                    progress_dialog.window.after(0, 
                        lambda: progress_dialog.add_log_message("INFO: Falling back to optimized CPU processing"))

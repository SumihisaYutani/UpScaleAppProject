...
class RealCUGANBackend:
    ...
    def set_thread_setting(self, thread_setting: str) -> bool:
        """Set the thread setting for Real-CUGAN processing"""
        logger.info(f"=== Real-CUGAN SET_THREAD_SETTING DEBUG START ===")
        ...
        if thread_setting in self.thread_options:
            ...
            logger.info(f"SUCCESS: Real-CUGAN thread setting changed from {old_setting} to: {thread_setting}")
            ...
            logger.info(f"=== Real-CUGAN SET_THREAD_SETTING DEBUG END ===")
            return True
        else:
